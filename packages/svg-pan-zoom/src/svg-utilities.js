...
export default {
   ...
   */
  getBoundingClientRectNormalized(svg) {
   ...
   */
  getOrCreateViewport(svg, selector) {
    let viewport = null;
    ...
    if (!viewport) {
      const viewportId = `viewport-${new Date()
        .toISOString()
        .replace(/\D/g, '')}`;
  ...
  refreshDefsGlobal: Utils.throttle(() => {
    ...
    const allDefsCount = allDefs.length;
    for (let i = 0; i < allDefsCount; i++) {
      const thisDefs = allDefs[i];
