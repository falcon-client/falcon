...
(function(global) {
  ...
  const urlParams = defined.document ? getUrlParams() : {};
  ...
  (function() {
    ...
    const onErrorFnPrev = window.onerror;
  ...
  Test.prototype = {
    ...
    hooks(handler) {
      ...
      if (
        ...
        hooks.push(
          this.queueHook(this.module.testEnvironment[handler], handler)
        );
      }
...
*/
  ...
  QUnit.pushFailure = function() {
    if (!QUnit.config.current) {
      throw new Error(
        `pushFailure() assertion outside test context, in ${sourceFromStacktrace(
          2
        )}`
      );
    }
    ...
    return currentTest.pushFailure(...arguments);
  };
  ...
  QUnit.assert = Assert.prototype = {
    ...
    ok(result, message) {
      message =
        ...
        (result
          ? 'okay'
          : `failed, expected argument to be truthy, was: ${QUnit.dump.parse(
              result
            )}`);
    ...
    notOk(result, message) {
      message =
        ...
        (!result
          ? 'okay'
          : `failed, expected argument to be falsy, was: ${QUnit.dump.parse(
              result
            )}`);
          ...
          node(node) {
            ...
            if (attrs) {
              for (i = 0, len = attrs.length; i < len; i++) {
                ...
                // "inherit".
                if (val && val !== 'inherit') {
                  ret += ` ${attrs[i].nodeName}=${dump.parse(
                    val,
                    'attribute'
                  )}`;
                }
  ...
  if (defined.document) {
    ...
    (function() {
      ...
      function applyCurrent(current) {
        return function() {
          const assert = new Assert(QUnit.config.current);
 ...
 */
  QUnit.diff = (function() {
    ...
    DiffMatchPatch.prototype.DiffMain = function(text1, text2, optChecklines) {
      let deadline, checklines, commonlength, commonprefix, commonsuffix, diffs;
    ...
    DiffMatchPatch.prototype.diffCommonOverlap = function(text1, text2) {
      let text1Length, text2Length, textLength, best, length, pattern, found;
      // Cache the text lengths to prevent multiple calls.
      text1Length = text1.length;
      text2Length = text2.length;
...
(function() {
  ...
  function getUrlConfigHtml() {
    ...
    for (i = 0; i < len; i++) {
      ...
      if (!val.value || typeof val.value === 'string') {
        urlConfigHtml += `<input id='qunit-urlconfig-${escaped}' name='${escaped}' type='checkbox'${
          val.value ? ` value='${escapeText(val.value)}'` : ''
        }${
          config[val.id] ? " checked='checked'" : ''
        } title='${escapedTooltip}' /><label for='qunit-urlconfig-${escaped}' title='${escapedTooltip}'>${
          val.label
        }</label>`;
      } else {
        urlConfigHtml += `<label for='qunit-urlconfig-${escaped}' title='${escapedTooltip}'>${
          val.label
        }: </label><select id='qunit-urlconfig-${escaped}' name='${escaped}' title='${escapedTooltip}'><option></option>`;
  ...
  function toolbarModuleFilterHtml() {
    ...
    modulesList.sort((a, b) => a.localeCompare(b));
    ...
    for (i = 0; i < modulesList.length; i++) {
      moduleFilterHtml += `<option value='${escapeText(
        encodeURIComponent(modulesList[i])
      )}' ${
  ...
  function appendHeader() {
    ...
    if (header) {
      header.innerHTML = `<a href='${setUrl({
        filter: undefined,
        module: undefined,
        testId: undefined
      })}'>${header.innerHTML}</a> `;
    ...
    if (!details.result && hasOwn.call(details, 'expected')) {
      ...
      if (actual !== expected) {
        ...
        message +=
          ...
          ` run with a higher max depth or <a href='${setUrl({
            maxDepth: -1
          })}'>` +
